        ...
        try:
            ...
            resp = requests.post(
                ...
                json={
                    ...
                    # [FIX-v339-THINKING] Корень ошибки «ответ без текстовых блоков:
                    # types=['thinking']». У claude-sonnet-5 расширенное мышление включено
                    # ПО УМОЛЧАНИЮ, когда параметр thinking не задан. При max_tokens=600
                    # (анализ убытка) всё мышление съедало весь бюджет токенов, и в ответе
                    # оставался ТОЛЬКО thinking-блок без единого text-блока — парсер получал
                    # types=['thinking'] и падал. Для утилитарных JSON-запросов (анализ
                    # убытка, оценка монеты) мышление не нужно: отключаем его явно, весь
                    # бюджет токенов идёт на реальный ответ, вызов дешевле и быстрее.
